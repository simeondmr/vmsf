...
uint32_t pop_32(uint8_t *ram, struct registers *regs) {
	regs->sp -= 4;
	return read_ram_32(ram, regs->sp);
}
...
int is_overflow(uint32_t a, uint32_t b, int carry, int neg) { // todo: check
	uint32_t r = calc_sum(a, b, carry, neg);
	return !(is_negative(a) ^ is_negative(b) ^ neg) && is_negative(r) ^ is_negative(a);
}

int is_subcarry(uint32_t a, uint32_t b, int carry, int neg) { // todo: check
	a=abs(a);
	b=abs(b);
	if(neg) {
		b=-b;
...
int is_carry(uint32_t a, uint32_t b, int carry, int neg) { // todo: check
	uint32_t r = calc_sum(a, b, carry, neg);
